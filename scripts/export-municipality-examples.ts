...
async function main() {
  try {
    console.log('Fetching one example tree from each municipality with internal ID...');
    ...
    // Save to file
    const outputPath = join(process.cwd(), 'municipality-examples.csv');
    ...
    results.slice(0, 5).forEach((row, i) => {
      console.log(`  ${i + 1}. ${row['muni_name']} (Tree ID: ${row['meta-tree-id']})`);
    });
